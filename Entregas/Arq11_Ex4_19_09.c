...
volatile unsigned char tecla_pressionada = TECS_OPEN;
...
unsigned char le_tec(void) __reentrant {
...
void main (void) {
	...
	printf_fast_f("\x01");
	while(1) {
		if(tecla_pressionada != TECS_OPEN)
			printf_fast_f("%d", tecla_pressionada);
		tecla_pressionada = TECS_OPEN;
	}
}
